...
def update_ai_processed(transaction_id, ai_processed):
    """به‌روزرسانی وضعیت پردازش AI تراکنش"""
    conn = None
    try:
        conn = create_connection()
        cursor = conn.cursor()
        ...
        if cursor.rowcount > 0:
            conn.commit()
            logger.info(f"وضعیت پردازش AI تراکنش {transaction_id} به {ai_processed_int} تغییر کرد")
        else:
            logger.warning(f"تراکنشی با شناسه {transaction_id} یافت نشد")
    except Exception as e:
        logger.error(f"خطا در به‌روزرسانی وضعیت پردازش AI تراکنش {transaction_id}: {str(e)}")
        if conn:
            conn.rollback()
        raise
    finally:
