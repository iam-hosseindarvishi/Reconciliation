...
"""
ماژول مغایرت‌گیری چک‌های دریافتی
این ماژول مسئول انجام مغایرت‌گیری تراکنش‌های چک دریافتی است.
"""
...
class ReceivedCheckReconciler:
    """
    کلاس مغایرت‌گیری چک‌های دریافتی
    """
    ...
    def reconcile(self, bank_record: Dict[str, Any]) -> bool:
        ...
        """
        try:
            logger.info(f"شروع مغایرت‌گیری چک دریافتی برای رکورد {bank_record.get('id')}")
            
            # دریافت ورودی‌های حسابداری چک
            accounting_entries = self._get_check_accounting_entries(bank_record, 'received')
            
            if not accounting_entries:
                logger.warning(f"ورودی‌های حسابداری چک دریافتی برای رکورد {bank_record.get('id')} یافت نشد")
                return False
                
            # تلاش برای تطبیق چک
            for entry in accounting_entries:
                check_match = self._find_check_match(bank_record, entry, 'received')
                if check_match:
                    return self._record_check_match(bank_record, entry, check_match, 'received')
                    
            logger.warning(f"تطبیق چک دریافتی برای رکورد {bank_record.get('id')} یافت نشد")
            return False
            
        except Exception as e:
            logger.error(f"خطا در مغایرت‌گیری چک دریافتی: {str(e)}")
            return False
